...
for v in data:
    ...
    if prev and not t:
        spans += 1
    prev = t
print(f'blit spans: {spans} (loader cap 449)')
assert spans <= 449, 'too many spans - simplify the transparent area'

with open(os.path.join(root, 'SRC', 'PANELIMG.INC'), 'w', encoding='ascii') as f:
...
with open(os.path.join(root, 'SRC', 'PALPNL.INC'), 'w', encoding='ascii') as f:

...
class Program
{
    ...
    static int RunEmit(string panelPath, string uiPath, string srcDir, string installDir)
    {
        ...
        // THE CLASSIC first (2026-08-18): mode 4, palette 1, intense -
        // black/cyan/magenta/white, the palette every CGA game wore.
        // (Mode 5's burst-off cyan/red/white was tried and renders
        // unpredictably - "народ не поймёт".) And the OTHER real one
        // behind it: palette 0 intense - green/red/yellow - because the
        // hardware has always had both and the H key now swaps them in
        // flight, lookup banks and palette register together.
        int[][] cgaPal1 = { new[]{0,0,0}, new[]{85,255,255}, new[]{255,85,255}, new[]{255,255,255} };
        int[][] cgaPal0 = { new[]{0,0,0}, new[]{85,255,85},  new[]{255,85,85},  new[]{255,255,85} };
        var sb = new StringBuilder();
        sb.Append("; VIDLUT.INC - generated by TOOLS\\VidRig emit. DO NOT EDIT BY HAND.\r\n");
        sb.Append("; The 256->4 lookup for the CGA converter, FOUR banks of 1024:\r\n");
        sb.Append(";   +0    palette 1 (cyan/magenta/white), day\r\n");
        sb.Append(";   +1024 palette 1, night\r\n");
        sb.Append(";   +2048 palette 0 (green/red/yellow), day\r\n");
        sb.Append(";   +3072 palette 0, night\r\n");
        sb.Append("; - four 256-byte phase tables each (phase = x&1 + (y&1)*2),\r\n");
        sb.Append("; 512-aligned so the blit walks phases with XOR on BH alone.\r\n");
        sb.Append("        align 512\r\n");
        sb.Append("VLUT:\r\n");
        foreach (var cgaSet in new[] { cgaPal1, cgaPal0 })
        foreach (var palName in new[] { "PALDAY", "PALNITE" })
        {
            ...
            var lut = BuildLut(rgb, cgaSet, dither: true);
            for (int phase = 0; phase < 4; phase++)
            {
                sb.Append($"; {(cgaSet == cgaPal1 ? "palette 1" : "palette 0")}, {palName}, phase {phase}\r\n");
